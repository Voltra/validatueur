...
export class SameAs<T = any> extends AbstractValidator<T, any> {
	...
	protected async __validate(
		...
		const { result: otherResult } = await schema.__validateField(otherField, schema.rawData[otherField]);
		const { result: thisResult } = await schema.__validateField(otherField, value);

		if(isError(otherResult) || isError(thisResult))
